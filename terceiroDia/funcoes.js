...
function aprender() {
  ...
  const aprender = document.querySelector('input[name="aprender"]:checked').value;
...
function enviartec() {
  ...
  for (var indice = 0; indice < listatecs.length; indice++) {
    ...
    document.getElementById(
      "digitartec"
    ).innerHTML = `<p> Existe outra tecnlogia que você gostaria de aprender?</p><input type='radio' name='tecnologias' value='sim' checked /><label for='sim'>Sim</label><input type='radio' name='tecnologias' value='nao' /><label for='nao'>Não</label><br><br><button type='submit' onclick='sn()'>Escolher</button>`;
  }
}
